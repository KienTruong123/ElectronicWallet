...
// }).save()
  ...
  return res.render('admin/admin_dashboard',{active_list,waiting_list,ban_list,lock_list,trade_list,admin: true});
});
    ...
    if(!result)
      {
        ...
        for(var i=0;i<trade_list.length;i++){
          ...
          else if(trade_list[i].type == 'Deposit'){
            //let sender =  await CreditCard.find({_id: trade_list[i].sender_id}).lean()
            let receiver = await User.find({_id: trade_list[i].receiver_id}).lean()
            trade_list[i].sender_name = "Bank"
        ...
        return res.render('admin/admin_edit_user',{user: user,trade_list: trade_list,admin: true})
      }
  })
  .catch(err=>{
    console.log(err)
...
router.post('/updateUser',async (req,res)=>{
  ...
  if(user.secure_status > 5){
    if(type == 'unlock'){
      ...
      const filter = { _id: id };
      const update = { secure_status: 0, password: bcrypt.hashSync(user.phone,5) };
      User.findOneAndUpdate(filter, update)
      .then(user=>{
        console.log("User ne")
        console.log(user)
      ...
      return res.json({valid: false,message:"User is "+user.status})
    }
  }
    ...
    if(!result)
      {
        ...
        for (var x in result) 
          ...
          if(result.type=='Withdraw'){
            ...
            let receiver = await CreditCard.find({_id: result.receiver_id}).lean()
            trade.sender_name = sender[0].name != undefined ? sender[0].name : "Unknown"
            trade.receiver_name = "Bank"
            ...
            trade.cardExpired = moment(receiver[0].expiredDate).format('L') +' '+ moment(receiver[0].expiredDate).format('LTS');
            trade.limit = receiver[0].limit == -1 ? "Không giới hạn" : receiver[0].limit 
            trade.receiver_email = "Bank email"
          }
            ...
            let sender =  await CreditCard.find({_id: result.sender_id}).lean()
            let receiver = await User.find({_id: result.receiver_id}).lean()
            trade.sender_name = "Bank"
            trade.receiver_name = receiver[0].name != undefined ? receiver[0].name : "Unknown"
            trade.sender_balance = sender[0].balance.toLocaleString('it-IT', {style : 'currency', currency : 'VND'});
            trade.receiver_balance = receiver[0].balance.toLocaleString('it-IT', {style : 'currency', currency : 'VND'});
            trade.sender_email = "Bank email"
            trade.cardExpired = moment(sender[0].expiredDate).format('L') +' '+ moment(sender[0].expiredDate).format('LTS');
            trade.limit = sender[0].limit == -1 ? "Không giới hạn" : sender[0].limit
            trade.receiver_email = receiver[0].email != undefined ? receiver[0].email : "Unknown"
          }
          //
